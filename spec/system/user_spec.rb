...
RSpec.feature 'ユーザ登録テスト', type: :system do
  ...
  describe 'ユーザ登録のテスト' do
    context '新規登録した場合' do
      it 'ユーザの新規登録ができる' do
        visit new_user_path
        fill_in '名前', with: 'fujii'
        fill_in 'メールアドレス', with: 'fujii@amail.com'
        fill_in 'パスワード', with: '1111ffff'
        fill_in '確認用パスワード', with: '1111ffff'
        click_button 'アカウントを作成'
        expect(page).to have_content 'fujiiのページ'
      end
    end
  ...
  describe '管理画面のテスト' do
    ...
    context 'ユーザーがアクセスした場合' do
      it '管理画面へは、管理者以外はアクセスできません' do
        visit new_session_path
        fill_in 'Email', with: 'aaa@amail.com'
        fill_in 'Password', with: '1111pppp'
        click_button 'Log in'
        click_on 'タスク一覧に戻る'
        click_link '管理画面'
